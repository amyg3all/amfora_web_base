...
export const HomeFontContext = createContext({
  filterOn: true,
...
const homeFontReducer = (state, action) => {
  switch (action.type) {
    case "setFilterOn":
...
const HomeFontContextProvider = ({ children }) => {
  const [fontState, dispatchFont] = useReducer(homeFontReducer, {
    filterOn: true,
    sizeLineDivider: SIZE_LINE_DIVIDER,
    charLineSpaceDivider: CHAR_LINE_SPACE_DIVIDER,
    fontSize: DEFAULT_FONT_SIZE,
  ...
  return (
    <HomeFontContext.Provider value={ctx}>{children}</HomeFontContext.Provider>
  );
};
export default HomeFontContextProvider;